...
if peregrine_rocket.is_file(): 	# if input file exists, use it
    # Read in the config file
    from peregrine_rocket import *
else:  # no input file exists; initialize input variables from the values below
	# Note: if input file exists, all of the values below will be ignored
    ...
    # predicted altitude at predicted maximum rocket velocity (AGL)
    AMaxV = 14000
    LSA = 4500          # launch site altitude (ASL)
    # Temp (Fahrenheit or Centigrade, depending upon selected units)
    TLS = 65
    # Use Default Sea-Level Temp ("DST") or Launch Site Temp ("LST")
    DEF = "DST"
    # Fin Specs
...
# delete third column of csv file (created by extra comma)
...
for i in range(len(verts)):    # create segments of a closed PSLG from list of vertices
    ...
    if (i == (len(verts)-1)):
        ...
        seg.append(i+1)
    segs.append(seg)
# triangle expects Python dict as input
tr_input = dict(vertices=verts, segments=segs)
# make triangles. 'p' says input is a PSLG
tr_output = tr.triangulate(tr_input, 'p')
# convert output of triangle to Python list
...
Term1 = (DN * pow(AspectRatio, 3)) / \
    (pow(ThicknessRatio, 3) * (AspectRatio + 2))
Term2 = (Lambda + 1)/2
Term3 = (airP / p0)
...
if (T2T == "YES"):
    print("Tip-to-tip reinforcing present; GE doubled to: ",
          "{:2.1f}".format(GE), GEsuf)
else:
...
# set up what we are going to plot
