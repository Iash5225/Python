from sympy import symbols, lambdify, sqrt, pi, Pow
import numpy as np
import math
...
flutter_velocity_function = lambdify(
    (airP, Temp, Spd_of_Sound, GE, RC, Height, TC, Thickness, fin_Cx), Vf_expr, modules='sympy')

# Lambdify the new expressions
calculate_terms_function = lambdify((MaxV, fin_Cx, RC, TC, Height, fin_area, p0, DEF, LSA, AMaxV, DST_Base_Temp, Temp_Dec_Per_Unit, TLS, SoS_Mult, Low_Temp, T0, Thickness, GE, airP), (
...
def main():
    ...
    # Read vertex data and calculate fin area and center of pressure
    triangles, verts = read_vertex_file(fin_vertex_file_name)
    fin_area_val, fin_cx_val, _, _ = calculate_triangle_areas_and_cxs(
        triangles, verts)
    ...
    # Calculate intermediate terms including air pressure
    terms = calculate_terms_function(maxv, fin_cx_val, rc, tc, height, fin_area_val, p0_val, "DST", lsa, amaxv,
                                     dst_base_temp_val, temp_dec_per_unit_val, tls, sos_mult_val, low_temp_val, t0_val, thickness, ge, 0)
    ...
    # Correctly calculate flutter velocity
    # In the main function, when calling calculate_flutter_velocity
    #flutter_velocity = calculate_flutter_velocity(
        #float(airp), float(temp_val), float(spd_of_sound), float(ge), float(rc), float(height), float(tc), float(thickness), float(fin_cx_val))

    # Calculate flutter velocity numerically
    flutter_velocity = Vf_expr.subs({
        SoS_Mult: sos_mult_val,
        fin_area: fin_area_val,
        p0: p0_val,
        AMaxV: amaxv,
        Temp_Dec_Per_Unit: temp_dec_per_unit_val,
        Low_Temp: low_temp_val,
        TLS: tls,
        T0: t0_val
    }).evalf()
    # Output the results
    print(f"Fin Area: {fin_area_val}, Center of Pressure: {fin_cx_val}, Flutter Velocity: {flutter_velocity}, Margin: {margin}, Margin Percentage: {margin_pct}%")
